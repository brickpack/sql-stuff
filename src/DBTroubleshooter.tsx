      ...
      sqlserver: [
        {
          ...
          checks: [
            { label: "Warning icons (yellow/red triangles) in plan?", key: "warnings", advice: "Investigate the specific warning. Common issues: implicit conversions (ensure parameter types match column types), missing indexes (create them), or missing statistics (run UPDATE STATISTICS)." },
            { label: "Estimate vs Actual rows >10x difference?", key: "estimates", advice: "Update statistics with FULLSCAN: UPDATE STATISTICS YourTable WITH FULLSCAN; If problem persists, check for stale statistics, parameter sniffing issues, or outdated cardinality estimates." },
            { label: "Table Scan on tables with >100k rows?", key: "tableScans", advice: "Add an index on columns used in WHERE, JOIN, or ORDER BY clauses. Use: CREATE NONCLUSTERED INDEX IX_YourIndex ON YourTable(FilterColumn) INCLUDE (SelectColumns);" },
            { label: "Many Key Lookups (>1000 rows)?", key: "keyLookups", advice: "Add INCLUDE columns to your index to create a covering index: CREATE NONCLUSTERED INDEX IX_Name ON Table(KeyColumns) INCLUDE (LookupColumns); This eliminates the need for key lookups." },
            { label: "Operators with >50% of total cost?", key: "highCost", advice: "Focus optimization on this operator. Common fixes: add indexes for scans/seeks, increase memory for sorts/hashes, or rewrite query to avoid expensive operations." },
            { label: "Thick arrows indicating millions of rows?", key: "thickArrows", advice: "Reduce row count earlier in query. Add WHERE filters before JOINs, use TOP/OFFSET-FETCH, or consider query redesign to process fewer rows." },
            { label: "Memory grants showing spills to tempdb?", key: "memorySpills", advice: "Increase memory grant with query hint: OPTION (MAX_GRANT_PERCENT = 25), update statistics for better estimates, or add indexes to reduce sort/hash size." },
            { label: "Sort or Hash operations with warnings?", key: "sortHash", advice: "Add indexes to eliminate sorts (covering index with columns in ORDER BY), increase memory grants, or reduce result set size before sorting/hashing." }
          ]
        },
        {
        ...
        },
        {
          ...
          checks: [
            { label: "PAGELATCH waits > 1000ms total?", key: "pagelatchWaits", advice: "Add more tempdb files (1 per CPU core, up to 8 initially). Ensure all files are equal size. Check sys.dm_os_wait_stats for PAGELATCH_UP/EX waits on resources 2:1:1 or 2:1:3." },
            { label: "Tempdb has fewer files than CPU cores?", key: "insufficientFiles", advice: "Add tempdb data files: ALTER DATABASE tempdb ADD FILE (NAME = tempdev2, FILENAME = 'path', SIZE = 8GB). Create 1 file per CPU core (max 8 for starters)." },
            { label: "Tempdb files are different sizes?", key: "unequalSizes", advice: "Resize all tempdb files to equal size: ALTER DATABASE tempdb MODIFY FILE (NAME = tempdev, SIZE = 8GB). Repeat for each file. Equal sizes ensure proper proportional fill algorithm." },
            { label: "High tempdb usage by specific sessions?", key: "highUsage", advice: "Identify the session using sys.dm_db_session_space_usage. Review the query causing high usage - optimize to reduce sorts, temp tables, or table variables. Consider killing the session if necessary." },
            { label: "Frequent tempdb autogrowth events?", key: "autogrowth", advice: "Pre-size tempdb files larger (8GB+ per file). Use fixed growth increments (512MB-1GB) not percentages. Monitor default trace or XEvents for autogrowth events." }
          ]
        },
        {
          ...
          checks: [
            { label: "Sessions blocked > 10 seconds?", key: "longBlocking", advice: "Identify the blocking session using sys.dm_exec_requests. Review the blocking query - add indexes, optimize, or shorten the transaction. Consider killing the blocking session (KILL session_id) if it's stuck." },
            { label: "Blocking chains with 3+ levels?", key: "deepChains", advice: "Find the head blocker (session with blocking_session_id = 0). This session is holding locks that cascade down. Review its query, add appropriate indexes, or commit/rollback the transaction." },
            { label: "Deadlocks occurring frequently?", key: "frequentDeadlocks", advice: "Enable trace flag 1222 to log deadlocks: DBCC TRACEON(1222, -1). Review deadlock graphs in system_health XEvents. Fix by accessing tables in same order, using proper indexes, or reducing transaction scope." },
            { label: "Lock escalation to table level?", key: "lockEscalation", advice: "Break large operations into smaller batches. Use ROWLOCK hint to prevent escalation: UPDATE table WITH (ROWLOCK). Consider disabling escalation: ALTER TABLE table SET (LOCK_ESCALATION = DISABLE) - use carefully." },
            { label: "Long-running transactions (>60s)?", key: "longTransactions", advice: "Identify using sys.dm_tran_active_transactions. Review application code - commit/rollback transactions promptly. Add indexes to speed up queries. Consider breaking into smaller transactions or using READ_COMMITTED_SNAPSHOT." }
          ]
        }
      ],
      postgresql: [
        ...
        },
        {
          ...
          checks: [
            { label: "Execution time > 1 second?", key: "slowExecution", advice: "Review the execution plan for expensive operations. Common fixes: add indexes for seq scans, increase work_mem for sorts/hashes, optimize WHERE clauses, or rewrite query logic." },
            { label: "Row estimates way off (10x+ difference)?", key: "badEstimates", advice: "Run ANALYZE on the affected tables to update statistics. If estimates are still off, consider increasing statistics target: ALTER TABLE table ALTER COLUMN col SET STATISTICS 1000; Then ANALYZE again." },
            { label: "Seq Scan on tables with 100k+ rows?", key: "seqScan", advice: "Add an index on columns used in WHERE, JOIN, or ORDER BY: CREATE INDEX CONCURRENTLY idx_name ON table(column). Use CONCURRENTLY to avoid blocking." },
            { label: "High buffer reads (not hits)?", key: "diskIO", advice: "Increase shared_buffers if <25% of RAM. Add indexes to reduce table scans. Check if working set fits in memory. Consider pg_prewarm to preload important tables into cache." },
            { label: "External merge or external sort present?", key: "externalSort", advice: "Increase work_mem for this session: SET work_mem = '256MB'. Or add index to avoid sort: CREATE INDEX ON table(order_by_column). Be careful: work_mem is per-operation, not per-query." },
            { label: "Nested Loop with loops > 1000?", key: "highLoops", advice: "Add index on the inner table's join column: CREATE INDEX CONCURRENTLY idx ON inner_table(join_col). Or force hash join if appropriate: SET enable_nestloop = off; (test only)." },
            { label: "Hash join with multiple batches?", key: "hashBatches", advice: "Increase work_mem to fit hash table in memory: SET work_mem = '512MB'. Check 'Buckets' vs 'Batches' in plan - multiple batches mean insufficient memory." }
          ]
        },
        {
        ...
        },
        {
          ...
          guidance: [
            ...
            "Requires track_io_timing = on in Parameter Group — columns are zero without it",
            "PG13-16: blk_read_time / blk_write_time — PG17+: shared_blk_read_time / shared_blk_write_time",
            "High read time / calls = query is I/O-bound, not CPU-bound",
        ...
        },
        {
          ...
          checks: [
            { label: "Dead tuple percentage > 20%?", key: "highDeadTuples", advice: "Run manual VACUUM ANALYZE on affected tables immediately. Tune autovacuum aggressively: ALTER TABLE table SET (autovacuum_vacuum_scale_factor = 0.05). Check for long-running transactions blocking vacuum." },
            { label: "Last autovacuum > 7 days ago?", key: "staleVacuum", advice: "Run VACUUM ANALYZE manually. Check autovacuum logs for errors. Verify autovacuum is enabled: SHOW autovacuum. Tune autovacuum_naptime and vacuum thresholds if needed." },
            { label: "Tables with >10GB size growing?", key: "largeTablesGrowing", advice: "Monitor dead tuple accumulation using pg_stat_user_tables. Consider VACUUM FULL during maintenance window to reclaim space (locks table). Investigate why bloat is occurring - long transactions or autovacuum not keeping up." },
            { label: "Long transactions (>1 hour) running?", key: "longTransactions", advice: "Identify using pg_stat_activity where xact_start is old. Long transactions prevent VACUUM from removing dead tuples. Review application code to commit/rollback promptly. Consider killing if stuck: pg_terminate_backend(pid)." },
            { label: "Autovacuum not running on busy tables?", key: "noAutovacuum", advice: "Check autovacuum_count in pg_stat_user_tables. Increase autovacuum workers: ALTER SYSTEM SET autovacuum_max_workers = 5. Lower scale_factor for busy tables: ALTER TABLE table SET (autovacuum_vacuum_scale_factor = 0.02)." }
          ]
        },
        {
          ...
          checks: [
            { label: "Connection usage > 80% of max_connections?", key: "highConnUsage", advice: "Implement connection pooling (PgBouncer) immediately. Terminate idle connections. Set connection limits per database/user. Consider increasing max_connections if you have sufficient RAM (~10MB per connection)." },
            { label: "Many idle in transaction connections?", key: "idleInTransaction", advice: "Set idle_in_transaction_session_timeout = '10min' in postgresql.conf. Review application code for missing commits/rollbacks. Terminate problematic connections: SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state = 'idle in transaction'." },
            { label: "Idle connections > 15 minutes old?", key: "longIdle", advice: "Configure connection pooling to close idle connections. Use PgBouncer with server_idle_timeout. Application should close connections when done. Terminate old idle connections: SELECT pg_terminate_backend(pid) WHERE state='idle' AND state_change < now() - interval '15 min'." },
            { label: "Connection errors in logs?", key: "connectionErrors", advice: "Check PostgreSQL logs for 'too many connections' errors. Review max_connections setting. Implement connection pooling (PgBouncer). Set per-database/user connection limits to prevent single app from consuming all connections." },
            { label: "No connection pooling in use?", key: "noPooling", advice: "Install and configure PgBouncer for connection pooling. Use pool_mode=transaction for most apps. Set appropriate pool sizes (default_pool_size=25). This drastically reduces connection overhead and prevents exhaustion." }
          ]
        },
        {
      ...
      mysql: [
        {
          ...
          checks: [
            { label: "type = ALL on tables >100k rows?", key: "tableScan", advice: "Add an index on columns in WHERE, JOIN, or ORDER BY: CREATE INDEX idx_name ON table(column). Use composite indexes for multiple columns. Check existing indexes with SHOW INDEXES FROM table." },
            { label: "Using filesort or Using temporary?", key: "tempSort", advice: "Add index on ORDER BY columns to eliminate filesort. For GROUP BY, add index on grouping columns. Increase sort_buffer_size if sorts are unavoidable. Consider covering index to avoid temp table." },
            { label: "High row count examined?", key: "highRows", advice: "Add more selective indexes. Review WHERE clause conditions - are they using indexes? Consider composite indexes matching your query patterns. Check for data type mismatches preventing index usage." },
            { label: "filtered < 10%?", key: "lowFilter", advice: "Low filtered% means poor index selectivity. Add more specific indexes, use composite indexes, or add WHERE conditions earlier in query. Update table statistics: ANALYZE TABLE table_name." },
            { label: "key column is NULL (no index used)?", key: "noIndex", advice: "No index being used! Create index on columns in WHERE/JOIN/ORDER BY: CREATE INDEX idx_name ON table(col1, col2). For covering index, add columns to avoid lookups: CREATE INDEX idx_name ON table(key_col, select_col)." },
            { label: "Extra shows 'Using where' without index?", key: "whereNoIndex", advice: "'Using where' without index means table scan with filter. Create index on WHERE columns: CREATE INDEX idx_name ON table(where_column). Ensure column types match to avoid implicit conversion." }
          ]
        },
        {
        ...
        },
        {
          ...
          checks: [
            { label: "Buffer pool hit ratio < 95%?", key: "lowHitRatio", advice: "Increase innodb_buffer_pool_size to 70-80% of available RAM: SET GLOBAL innodb_buffer_pool_size = 8589934592; (8GB). Add indexes to reduce full table scans. Check if working set fits in memory." },
            { label: "Buffer pool utilization > 90%?", key: "highUtilization", advice: "Buffer pool is nearly full. Consider increasing innodb_buffer_pool_size if RAM available. Monitor cache misses. If hit ratio is good (>99%), current size may be adequate despite high utilization." },
            { label: "Frequent disk reads (high Innodb_buffer_pool_reads)?", key: "frequentDiskReads", advice: "Increase buffer pool size to cache more data. Add indexes to reduce data accessed. Optimize queries to access less data. Check if specific tables cause most reads - consider partitioning or archiving old data." },
            { label: "Buffer pool size < 70% of RAM?", key: "undersizedPool", advice: "Increase innodb_buffer_pool_size dynamically: SET GLOBAL innodb_buffer_pool_size = X; Target 70-80% of RAM for dedicated database servers. Leave RAM for OS, connections, and temp operations." },
            { label: "Large tables not fully cached?", key: "uncachedTables", advice: "Increase buffer pool if RAM allows. For very large tables that don't fit, ensure frequently accessed rows are cached. Add indexes to reduce full scans. Consider table partitioning to reduce working set size." }
          ]
        },
        {
          ...
          checks: [
            { label: "Seconds_Behind_Master > 10 seconds?", key: "replicationLag", advice: "Enable parallel replication: slave_parallel_workers=4, slave_parallel_type=LOGICAL_CLOCK in my.cnf. Check if replica hardware is slower than source. Reduce read load on replica. Optimize long-running queries on source." },
            { label: "IO or SQL thread not running?", key: "threadDown", advice: "Check Last_Error in SHOW REPLICA STATUS for reason. Common: connection issues (IO thread), duplicate key errors (SQL thread). Fix the error, then START REPLICA. May need to SKIP error or resync." },
            { label: "Last_Error is not empty?", key: "replicationError", advice: "Review the error message. Common fixes: Skip duplicate key error with sql_slave_skip_counter=1 (careful!), fix connection issues, resync replica if corrupted. Check error logs for details." },
            { label: "Parallel replication not enabled?", key: "noParallelReplication", advice: "Enable in my.cnf on replica: slave_parallel_workers=4 (or # of CPU cores), slave_parallel_type=LOGICAL_CLOCK, slave_preserve_commit_order=ON. Restart MySQL. This can dramatically reduce lag." },
            { label: "Relay log space running low?", key: "relayLogSpace", advice: "Check relay_log_space_limit and disk space. Increase relay_log_space_limit if needed. Ensure replica is processing relay logs (SQL thread running). Check for I/O issues. Clear old relay logs if SQL thread caught up." }
          ]
        }
      ],
      oracle: [
        {
          ...
          checks: [
            { label: "Execution cost > 10000?", key: "highCost", advice: "Review execution plan for expensive operations. Add indexes for full table scans. Gather statistics: DBMS_STATS.GATHER_TABLE_STATS. Consider hints (INDEX, USE_NL, USE_HASH) if optimizer chooses poorly." },
            { label: "Row estimates way off (E-Rows vs A-Rows)?", key: "badEstimates", advice: "Gather fresh statistics: EXEC DBMS_STATS.GATHER_TABLE_STATS(ownname=>'SCHEMA', tabname=>'TABLE', cascade=>TRUE). If estimates still off, increase sample size or use compute statistics." },
            { label: "Full table scan on large tables?", key: "fullScan", advice: "Create index on columns in WHERE/JOIN: CREATE INDEX idx_name ON table(column). Check if existing indexes can be used. Use hints if optimizer ignores index: /*+ INDEX(table idx_name) */." },
            { label: "Cartesian join in plan?", key: "cartesian", advice: "Missing JOIN condition! Review query for missing ON/WHERE clauses between tables. This causes explosive row multiplication. Add proper join conditions immediately." },
            { label: "TABLE ACCESS FULL on tables >100k rows?", key: "largeScan", advice: "Add index on columns used in WHERE, JOIN, ORDER BY: CREATE INDEX idx ON table(col). Gather statistics. If index exists but not used, check for functions on indexed columns or data type mismatches." },
            { label: "High A-Time (actual time) values?", key: "highTime", advice: "Focus on operations with highest A-Time. Common fixes: add indexes, increase PGA for sorts/hashes, optimize joins, reduce data processed. Check for I/O bottlenecks (db file sequential read waits)." }
          ]
        },
        {
        ...
        },
        {
          ...
          checks: [
            { label: "Cartesian join present?", key: "cartesianJoin", advice: "Missing join condition! Review query - ensure all tables have proper ON or WHERE join conditions. Cartesian joins multiply rows explosively. Add missing join predicates immediately." },
            { label: "Hash join running out of memory?", key: "hashMemory", advice: "Increase PGA_AGGREGATE_TARGET: ALTER SYSTEM SET pga_aggregate_target = 4G SCOPE=BOTH. Check v$pgastat for PGA usage. Consider partitioning large tables or rewriting query to reduce hash size." },
            { label: "Nested loop with many iterations?", key: "nestedLoop", advice: "Add index on inner table join column. High iterations mean repeated table access. CREATE INDEX idx ON inner_table(join_key). Or force hash join with hint: /*+ USE_HASH(table1 table2) */." }
          ]
        },
        {
        ...
        },
        {
          ...
          checks: [
            { label: "High db file sequential read waits?", key: "sequentialReadWaits", advice: "Single-block I/O waits (index/ROWID reads). Add indexes to reduce lookups. Check storage performance. Consider SSD/faster storage. Review queries with high logical reads in v$sql." },
            { label: "High db file scattered read waits?", key: "scatteredReadWaits", advice: "Multi-block I/O waits (full table scans). Add indexes to eliminate scans. Partition large tables. Increase DB_FILE_MULTIBLOCK_READ_COUNT if scans are unavoidable. Check v$sql for high disk reads." },
            { label: "High log file sync waits?", key: "logSyncWaits", advice: "Commit waits for redo log writes. Move redo logs to faster storage (dedicated SSD). Batch commits when possible. Increase LOG_BUFFER. Check LGWR performance. Consider commit NOWAIT for async commits (careful!)." },
            { label: "Enqueue or latch contention?", key: "concurrencyWaits", advice: "Lock/latch contention. For TX enqueues: shorten transactions, avoid hot rows. For latches: use bind variables (reduce hard parsing), tune shared pool. Check v$session for blocking. Use AWR for historical analysis." },
            { label: "Sessions waiting > 10 seconds?", key: "longWaits", advice: "Identify waiting session in v$session. Check event column for wait type. Common: Lock waits (kill blocker or optimize), I/O waits (add indexes), CPU waits (tune query). Consider killing stuck sessions." }
          ]
        },
        {
          ...
          checks: [
            { label: "Temp tablespace >80% full?", key: "tempFull", advice: "Add more tempfiles: ALTER TABLESPACE TEMP ADD TEMPFILE SIZE 1G AUTOEXTEND ON. Increase PGA_AGGREGATE_TARGET to reduce temp spills. Identify queries using v$tempseg_usage and optimize them." },
            { label: "Sessions using >1GB temp space?", key: "highTempUsage", advice: "Find session in v$tempseg_usage. Review the query - optimize to reduce sorts/hashes. Add indexes to avoid sorting. Increase PGA if unavoidable: ALTER SYSTEM SET pga_aggregate_target = 4G. Consider killing session if necessary." },
            { label: "High 'direct path write temp' waits?", key: "tempWriteWaits", advice: "Operations spilling to temp tablespace. Increase PGA_AGGREGATE_TARGET: ALTER SYSTEM SET pga_aggregate_target = 4G. Add indexes to avoid large sorts. Optimize queries to reduce intermediate result sets." },
            { label: "ORA-01652 errors in alert log?", key: "tempExtendErrors", advice: "Temp tablespace out of space! Add tempfiles immediately: ALTER TABLESPACE TEMP ADD TEMPFILE SIZE 5G AUTOEXTEND ON MAXSIZE 10G. Increase PGA to reduce temp usage. Check for runaway queries." },
            { label: "PGA_AGGREGATE_TARGET too small?", key: "lowPGA", advice: "Increase PGA: ALTER SYSTEM SET pga_aggregate_target = 4G SCOPE=BOTH (adjust based on RAM). Check v$pgastat for current usage. Rule of thumb: 20-30% of RAM for PGA. Ensure workarea_size_policy = AUTO." }
          ]
        },
        {
          ...
          checks: [
            { label: "AWR snapshots being collected?", key: "awrEnabled", advice: "Enable AWR snapshots: Check dba_hist_snapshot. If empty, verify Diagnostics Pack license and that STATISTICS_LEVEL = TYPICAL or ALL. AWR collects automatically every hour by default." },
            { label: "Recent ADDM findings available?", key: "addmFindings", advice: "Generate AWR report for problem period: @?/rdbms/admin/awrrpt.sql. Review ADDM findings in dba_advisor_findings. ADDM runs automatically after snapshots. Check for high-impact findings (DB time impact)." },
            { label: "AWR retention too short (<7 days)?", key: "shortRetention", advice: "Increase retention: EXEC DBMS_WORKLOAD_REPOSITORY.MODIFY_SNAPSHOT_SETTINGS(retention => 14400); (10 days in minutes). Consider storage implications. Longer retention = better historical analysis." },
            { label: "High-impact ADDM findings unresolved?", key: "criticalFindings", advice: "Review ADDM recommendations in dba_advisor_findings. Focus on findings with high 'impact' (DB time). Implement recommended fixes: add indexes, tune SQL, adjust config. Re-run ADDM after changes to verify improvement." },
            { label: "Diagnostics Pack licensed?", key: "diagnosticsLicense", advice: "Verify Oracle Diagnostics Pack license before using AWR/ADDM (extra cost). Check with Oracle sales/licensing. Alternative: Use Statspack (free but manual, less comprehensive). Review v$option for enabled features." }
          ]
        }
      ],
      snowflake: [
        {
          ...
          guidance: [
            ...
            "Operations: Execution steps (TableScan, Join, Aggregate, etc.)",
            "Pruning: Shows micro-partitions pruned vs scanned",
            "Cost: Relative cost estimates for operations",
            "",
            "💡 Snowflake-Specific Operations:",
            "TableScan: Reading from table (check partitionsTotal vs partitionsAssigned)",
            "Join: Hash join, nested loop, or merge join",
            "Aggregate: GROUP BY operations",
            "WindowFunction: OVER clause operations",
            "Result: Final output operation",
            "",
            "💡 Key Metrics to Check:",
            "partitionsTotal: Total micro-partitions in table",
            "partitionsAssigned: Micro-partitions that will be scanned (lower is better)",
            "Pruning ratio: (Total - Assigned) / Total × 100% (higher is better)",
            "bytes: Amount of data to process",
            "",
            "💡 Pruning Effectiveness:",
            ">90% pruned = excellent (clustering working well)",
            "50-90% pruned = good (may benefit from better clustering)",
            "<50% pruned = poor (needs clustering keys or better WHERE clauses)",
            "",
            "⚠️ Note: EXPLAIN shows estimates. Use Query Profile in Snowflake UI for actual execution statistics after running the query."
          ],
          checks: [
            { label: "Poor partition pruning (<50% pruned)?", key: "poorPruning", advice: "Add clustering keys: ALTER TABLE table_name CLUSTER BY (date_column, filter_column). Use columns in WHERE clauses. Avoid functions on clustered columns (DATE(col) prevents pruning). Check SYSTEM$CLUSTERING_INFORMATION('table_name') for cluster health." },
            { label: "Large table scan (>1000 partitions)?", key: "largeTableScan", advice: "Improve WHERE clause selectivity. Add clustering keys on filtered columns. Consider search optimization service: ALTER TABLE table_name ADD SEARCH OPTIMIZATION. Use TABLESAMPLE for approximate queries. Check if LIMIT can be pushed down." },
            { label: "Multiple table scans of same table?", key: "multipleScans", advice: "Rewrite query to scan table once. Use CTEs to materialize intermediate results. Consider UNION ALL instead of OR conditions. Ensure subqueries aren't causing repeated scans. Check for correlated subqueries." },
            { label: "Cartesian join detected?", key: "cartesianJoin", advice: "Add proper JOIN conditions. Ensure all tables have join predicates. Review WHERE clause for missing join logic. Cartesian joins are extremely expensive. Check for missing ON clauses or CROSS JOIN usage." },
            { label: "Large number of join steps?", key: "manyJoins", advice: "Review join order - Snowflake optimizes automatically but verify. Consider denormalizing for frequently joined tables. Break complex queries into CTEs. Use EXPLAIN to verify join strategy (Hash vs Nested Loop)." },
            { label: "Window function on large dataset?", key: "expensiveWindow", advice: "Add PARTITION BY to limit window scope. Ensure ORDER BY uses clustered columns. Consider if aggregate query can replace window function. Use QUALIFY to filter window results efficiently. Add clustering on PARTITION BY columns." },
            { label: "External table scan?", key: "externalScan", advice: "External tables (S3/Azure/GCS) are slower than native tables. Load frequently accessed data into Snowflake tables. Use optimal file formats (Parquet/ORC > CSV). Partition external files by directory structure matching filters." }
          ]
        },
        {
        ...
        },
        {
          ...
          checks: [
            { label: "Low cache hit ratio (<50%)?", key: "lowCacheHitRatio", advice: "Standardize query formatting (whitespace matters!). Avoid CURRENT_TIMESTAMP/RANDOM functions. Use query templates/views for common patterns. Keep warehouses running longer to maintain warehouse cache. Check bytes_scanned=0 in query_history." },
            { label: "USE_CACHED_RESULT disabled?", key: "cachingDisabled", advice: "Enable result caching: ALTER SESSION SET USE_CACHED_RESULT = TRUE (default). Verify with SHOW PARAMETERS LIKE 'USE_CACHED_RESULT'. Result cache is free (no compute cost) and lasts 24 hours." },
            { label: "Queries with time functions bypassing cache?", key: "timeFunctions", advice: "CURRENT_TIMESTAMP, CURRENT_DATE, RANDOM() prevent result caching. Parameterize time values when possible. Use fixed timestamps for reports. Consider materialized views for time-dependent aggregations." },
            { label: "Inconsistent query formatting?", key: "inconsistentQueries", advice: "Exact query text match required for cache hit (spaces/comments matter). Create views or stored procedures for common queries. Use query templates. Document standard formatting in team guidelines." },
            { label: "Warehouses suspending too frequently?", key: "frequentSuspend", advice: "Increase AUTO_SUSPEND timeout: ALTER WAREHOUSE wh SET AUTO_SUSPEND = 600 (10 min). Longer suspend = better warehouse cache retention. Balance cost vs cache benefits. Monitor credit usage vs performance gain." }
          ]
        },
        {
          ...
          checks: [
            { label: "High Time Travel storage costs (>20% of active)?", key: "highTimeTravelStorage", advice: "Reduce retention on non-critical tables: ALTER TABLE table SET DATA_RETENTION_TIME_IN_DAYS = 1. Use TRANSIENT tables for staging data. Create clones for long-term snapshots instead of high retention. Monitor TABLE_STORAGE_METRICS." },
            { label: "Retention period longer than needed?", key: "excessiveRetention", advice: "Audit table retention settings. Reduce to minimum needed for compliance/recovery: ALTER TABLE table SET DATA_RETENTION_TIME_IN_DAYS = 7. Standard edition max is 1 day. Enterprise can go up to 90 days." },
            { label: "Non-critical tables with high retention?", key: "unnecessaryRetention", advice: "Identify non-critical tables (staging, logs, temp data). Set minimal retention: ALTER TABLE table SET DATA_RETENTION_TIME_IN_DAYS = 1. Use TRANSIENT tables for ephemeral data (no Fail-safe, lower cost)." },
            { label: "Staging/temp tables not using TRANSIENT?", key: "missingTransient", advice: "Use TRANSIENT for temporary/staging tables: CREATE TRANSIENT TABLE staging (...). No Fail-safe = lower storage cost. Appropriate for ETL intermediates, temp results, staging areas. Not for production data." },
            { label: "Time Travel queries performing slowly?", key: "slowTimeTravelQueries", advice: "Time Travel queries can't use result cache and may scan more partitions. Add clustering keys to historical tables. Limit time offset when possible. Consider creating materialized snapshots (CLONE) for frequently accessed historical points." }
          ]
        }
      ]
    };
      ...
      <div className="min-h-screen bg-gray-200">
        <div className="max-w-4xl mx-auto p-6">
          <div className="text-center mb-8 pt-12">
            ...
            <h1 className="text-3xl font-bold mb-2 text-gray-700">Database Query Troubleshooter</h1>
            <p className="text-gray-600">Interactive guide to diagnose and fix slow queries</p>
          </div>
          <div className="mb-6">
            <label className="block text-sm font-medium mb-3 text-gray-600">Select Your Database Type:</label>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {['postgresql', 'mysql', 'sqlserver', 'oracle', 'snowflake'].map((db) => (
                <button key={db} onClick={() => setDbType(db)} className="p-6 bg-gray-50 border border-gray-400 rounded-lg hover:border-gray-500 hover:shadow-md transition-all duration-200">
                  <Database className="w-8 h-8 mx-auto mb-2 text-gray-500" />
                  <div className="font-semibold text-gray-700 text-sm">{db === 'sqlserver' ? 'SQL Server' : db === 'postgresql' ? 'PostgreSQL' : db === 'oracle' ? 'Oracle' : db === 'snowflake' ? 'Snowflake' : 'MySQL'}</div>
                </button>
              ))}
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gray-200">
      <div className="max-w-6xl mx-auto p-6">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <button onClick={() => { setDbType(''); setCurrentStep(0); setFindings({}); }} className="text-sm text-blue-600 hover:text-blue-700 mb-2 transition-colors duration-200">← Change Database</button>
            <h1 className="text-2xl font-bold text-gray-700">{dbType === 'sqlserver' ? 'SQL Server' : dbType === 'postgresql' ? 'PostgreSQL' : dbType === 'oracle' ? 'Oracle' : dbType === 'snowflake' ? 'Snowflake' : 'MySQL'} Troubleshooting</h1>
          </div>
          <div className="text-sm text-gray-600">Step {currentStep + 1} of {steps.length}</div>
        </div>

        <div className="mb-6 bg-amber-50/60 border-l-4 border-amber-400 p-4 rounded">
        ...
        <div className="mb-6"><div className="flex gap-2">{steps.map((_: Step, idx: number) => (<div key={idx} className={`flex-1 h-2 rounded ${idx <= currentStep ? 'bg-blue-500' : 'bg-gray-400'}`} />))}</div></div>

        <div className="bg-gray-50 border border-gray-400 rounded-lg shadow-sm">
          <div className="bg-slate-600 text-white p-5 rounded-t-lg">
            <h2 className="text-xl font-semibold">{currentStepData?.title}</h2>
            <p className="text-gray-100 text-sm mt-1">{currentStepData?.description}</p>
          </div>

        <div className="p-6">
          {currentStepData?.sql && (
            <div className="mb-6">
              {currentStepData?.warning && (<div className="mb-3 bg-orange-50 border-l-4 border-orange-500 p-3 rounded"><p className="text-sm text-orange-800 font-medium">{currentStepData.warning}</p></div>)}
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-semibold flex items-center gap-2 text-gray-600"><Play className="w-4 h-4 text-gray-600" />Run This Query:</h3>
                <button onClick={() => copyToClipboard(currentStepData.sql || '', 'sql')} className="flex items-center gap-1 text-sm text-blue-600 hover:text-blue-700 transition-colors duration-200"><Copy className="w-4 h-4" />{copiedIndex === 'sql' ? 'Copied!' : 'Copy'}</button>
              </div>
              <pre className="bg-gray-800 text-gray-100 p-4 rounded overflow-x-auto text-sm">{currentStepData.sql}</pre>
            </div>
          )}

          {currentStepData?.checks && (<div className="mb-6 bg-yellow-100/80 border border-yellow-400/60 rounded-lg p-4"><h3 className="font-semibold mb-3 flex items-center gap-2 text-yellow-800"><AlertCircle className="w-5 h-5 text-yellow-700" />Check for These Issues:</h3><div className="space-y-3">{currentStepData.checks.map((check: Check, idx: number) => (<div key={idx} className="space-y-2"><div className="flex items-center gap-2"><input type="checkbox" id={`check-${idx}`} checked={findings[check.key] || false} onChange={(e) => setFindings(prev => ({ ...prev, [check.key]: e.target.checked }))} className="w-4 h-4 accent-blue-500" /><label htmlFor={`check-${idx}`} className="text-sm text-gray-600">{check.label}</label></div>{findings[check.key] && (<div className="ml-6 pl-4 border-l-2 border-green-600 bg-green-100/80 p-3 rounded"><div className="flex items-start gap-2 mb-1"><CheckCircle className="w-4 h-4 text-green-700 flex-shrink-0 mt-0.5" /><span className="text-xs font-semibold text-green-800 uppercase">Advice:</span></div><p className="text-sm text-gray-600">{check.advice}</p></div>)}</div>))}</div></div>)}

          {currentStepData?.action && (<div className="mb-6"><div className="flex items-center justify-between mb-2"><h3 className="font-semibold flex items-center gap-2 text-gray-600"><CheckCircle className="w-4 h-4 text-green-700" />Fix With:</h3><button onClick={() => copyToClipboard(currentStepData.action || '', 'action')} className="flex items-center gap-1 text-sm text-blue-600 hover:text-blue-700 transition-colors duration-200"><Copy className="w-4 h-4" />{copiedIndex === 'action' ? 'Copied!' : 'Copy'}</button></div><pre className="bg-green-100/80 border border-green-400/60 p-4 rounded-lg overflow-x-auto text-sm text-gray-600">{currentStepData.action}</pre></div>)}

          {currentStepData?.examples && (<div className="mb-6"><h3 className="font-semibold mb-3 text-gray-600">Common Patterns to Fix:</h3>{currentStepData.examples.map((example: Example, idx: number) => (<div key={idx} className="mb-4 border-l-4 border-red-400 pl-4 bg-gray-100 p-3 rounded-r-lg"><div className="mb-2"><span className="text-xs font-semibold text-red-700 uppercase">❌ Bad:</span><pre className="bg-red-100/80 border border-red-400/60 p-2 rounded text-xs mt-1 overflow-x-auto text-gray-600">{example.bad}</pre></div><div className="mb-2"><span className="text-xs font-semibold text-green-700 uppercase">✅ Good:</span><pre className="bg-green-100/80 border border-green-400/60 p-2 rounded text-xs mt-1 overflow-x-auto text-gray-600">{example.good}</pre></div>{example.why && (<div className="mt-2 text-xs text-gray-600 italic">💡 Why: {example.why}</div>)}</div>))}</div>)}

          {currentStepData?.guidance && (<div className="bg-blue-100/80 border border-blue-400/60 rounded-lg p-4"><h3 className="font-semibold mb-2 text-blue-800">💡 Key Points:</h3><div className="space-y-2">{currentStepData.guidance.filter((point: string) => point.trim() !== '').map((point: string, idx: number) => {
            const isHeader = point.match(/^[💡⚠️⚙️🔧📊✅❌]/);
            return isHeader ? (
              <div key={idx} className="text-sm font-semibold text-gray-700 mt-3 first:mt-0">{point}</div>
            ) : (
              <div key={idx} className="text-sm text-gray-600 ml-4">{point}</div>
            );
          })}</div></div>)}
        </div>
      </div>
      ...
      <div className="mt-6 flex justify-between">
        <button onClick={() => setCurrentStep(Math.max(0, currentStep - 1))} disabled={currentStep === 0} className="px-6 py-3 border border-gray-500 bg-gray-50 text-gray-600 rounded-lg hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200">← Previous</button>
        <button onClick={() => setCurrentStep(Math.min(steps.length - 1, currentStep + 1))} disabled={currentStep === steps.length - 1} className="px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-all duration-200">Next <ChevronRight className="w-4 h-4" /></button>
      </div>
      </div>
    </div>
  );
